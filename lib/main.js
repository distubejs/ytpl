...
const getContinuationToken = item => {
	...
	try {
		...
		if (renderer.continuationEndpoint && renderer.continuationEndpoint.continuationCommand && renderer.continuationEndpoint.continuationCommand.token) {
			return renderer.continuationEndpoint.continuationCommand.token;
		}

		// Try alternative paths that YouTube sometimes uses
		if (renderer.button && renderer.button.buttonRenderer && renderer.button.buttonRenderer.command && renderer.button.buttonRenderer.command.continuationCommand && renderer.button.buttonRenderer.command.continuationCommand.token) {
			return renderer.button.buttonRenderer.command.continuationCommand.token;
		}

		// Another alternative path
		if (renderer.trigger && renderer.trigger.continuationCommand && renderer.trigger.continuationCommand.token) {
		...
		const recursiveToken = findTokenRecursively(renderer);
		if (recursiveToken) return recursiveToken;

		return null;
	} catch (e) {
		console.warn("Error extracting continuation token:", e);
		return null;
	}
};
...
const main = (module.exports = async (linkOrId, options, rt = 3) => {
	...
	try {
		...
		const thumbnail = (info.thumbnailRenderer.playlistVideoThumbnailRenderer || info.thumbnailRenderer.playlistCustomThumbnailRenderer).thumbnail.thumbnails.sort((a, b) => b.width - a.width)[0];
		...
		if (continuation) {
			...
			if (!token) {
				...
				console.warn("Could not find continuation token. Continuation structure:", JSON.stringify(continuation, null, 2));
			}
		} else {
			// Check if there are any continuation-like items we might have missed
			const allContinuations = rawVideoList.filter(x => Object.keys(x)[0].toLowerCase().includes("continuation") || (x.continuationItemRenderer && Object.keys(x.continuationItemRenderer).length > 0));
...
const parsePage2 = async (apiKey, token, context, opts) => {
	...
	if (continuation) {
		...
		if (!nextToken) {
			console.warn("Could not find next continuation token. Continuation structure:", JSON.stringify(continuation, null, 2));
		}
	} else {
		// Check if there are any continuation-like items we might have missed
		const allContinuations = wrapper.filter(x => Object.keys(x)[0].toLowerCase().includes("continuation") || (x.continuationItemRenderer && Object.keys(x.continuationItemRenderer).length > 0));
		if (allContinuations.length > 0) {
			console.warn(
