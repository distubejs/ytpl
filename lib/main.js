...
const getContinuationToken = (item) => {
  ...
  try {
    ...
    if (renderer.continuationEndpoint && 
    ...
    if (renderer.button && 
        ...
        renderer.button.buttonRenderer.command.continuationCommand &&
    ...
    if (renderer.trigger &&
        renderer.trigger.continuationCommand &&
        renderer.trigger.continuationCommand.token) {
...
const main = module.exports = async (linkOrId, options, rt = 3) => {
  ...
  try {
    ...
    if (continuation) {
      ...
      const allContinuations = rawVideoList.filter(x => 
      ...
      if (allContinuations.length > 0) {
        console.warn('Found potential continuation items:', allContinuations.map(x => Object.keys(x)[0]));
      }
    }
...
const parsePage2 = async (apiKey, token, context, opts) => {
  ...
  if (continuation) {
    ...
    // Check if there are any continuation-like items we might have missed
    const allContinuations = wrapper.filter(x => 
      Object.keys(x)[0].toLowerCase().includes('continuation') || 
      (x.continuationItemRenderer && Object.keys(x.continuationItemRenderer).length > 0)
    );
    if (allContinuations.length > 0) {
      console.warn('Found potential next continuation items:', allContinuations.map(x => Object.keys(x)[0]));
    }
  }
